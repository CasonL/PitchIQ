...
export const NovaSonicInterface: React.FC<NovaSonicInterfaceProps> = ({ 
  ...
  const isPlayingRef = useRef(false); // Immediate playing state tracking
    ...
    if ('speechSynthesis' in window) {
      ...
      utterance.onend = () => {
        ...
        setTimeout(() => {
          ...
          console.log('🎤 Browser speech synthesis cooldown complete - recording can resume');
        }, 1000);
      };
...
After the salesperson responds, continue the conversation naturally based on their approach. Be challenging but realistic, ask relevant questions, and provide appropriate objections to help them practice their sales skills. Speak naturally and conversationally.`;
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        if (recordingMode === 'auto' && !isPlayingRef.current) {
          setTimeout(() => {
            if (!isPlayingRef.current) {
      ...
      if (response.ok) {
        ...
        if (data.success && data.responses && data.responses.length > 0) {
          ...
          for (const responseItem of data.responses) {
            ...
            console.log('🔍 Audio check - audio exists:', !!responseItem.audio, 'audio length:', responseItem.audio?.length || 0);
  ...
  const playNovaAudioResponse = async (audioBase64: string) => {
    ...
    try {
      ...
      // Nova Sonic returns MP3 audio from OpenAI TTS - create appropriate blob
      ...
      audio.onended = () => {
        ...
        setTimeout(() => {
          setIsPlaying(false);
          isPlayingRef.current = false; // Update ref immediately
          console.log('🎤 Nova Sonic audio ended - recording can resume');
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <Card>
        ...
        <CardContent>
          ...
          {/* OpenAI Real-Time API Test Section */}
          <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-200">
            <h3 className="text-lg font-semibold mb-3 text-blue-800">🚀 OpenAI Real-Time API (Ultra-Low Latency)</h3>
            <p className="text-sm text-blue-700 mb-3">
              Test the new OpenAI Real-Time API with ~500ms response time vs Nova Sonic's 20-30 second delays.
            </p>
              ...
              <Button
                onClick={async () => {
                  try {
                    ...
                    const data = await response.json();
                    if (data.success) {
                      alert(`✅ OpenAI Real-Time API SUCCESS!\nSession ID: ${data.session_id}\n\nThis will replace Nova Sonic with ultra-fast voice conversations!`);
                    } else {
                      alert(`❌ OpenAI Real-Time API FAILED: ${data.error}`);
                    }
                  } catch (err: any) {
                    alert(`❌ OpenAI Real-Time API ERROR: ${err.message}`);
                  }
                }}
                className="bg-blue-600 hover:bg-blue-700"
              >
                Test OpenAI Real-Time API
              </Button>
              ...
              <Button
                onClick={async () => {
                  try {
                    ...
                    const data = await response.json();
                    if (data.success) {
                      alert(`✅ OpenAI Real-Time Status:\nConnected: ${data.status.connected}\nActive Sessions: ${data.status.active_sessions}`);
                    } else {
            ...
            <div className="mt-3 text-xs text-blue-600 space-y-1">
              <p>• Bidirectional streaming with real-time audio processing</p>
              <p>• Supports interruption and natural conversation flow</p>
              <p>• No more waiting 20-30 seconds for responses!</p>
            </div>
          </div>
